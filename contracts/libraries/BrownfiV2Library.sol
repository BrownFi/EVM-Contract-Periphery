...
library BrownfiV2Library {
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IBrownfiV2Pair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        ...
        require(reserveA > 0 && reserveB > 0, 'BrownfiV2Library: INSUFFICIENT_LIQUIDITY');
    ...
    // post trade price
    function postP(uint reserveIn, uint reserveOut, uint amountOut, uint k) internal pure returns (uint p){
